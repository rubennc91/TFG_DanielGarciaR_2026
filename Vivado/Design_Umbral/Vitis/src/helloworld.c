...
// Convertir un byte en una cadena de 1 y 0
...
*/
	...
	// =========================================================================
		// --- CONFIGURACIÓN DE UMBRALES DEL FIR A 0.5 (Para Q_suma: <32, 12>) ---
		// Reducimos el umbral a ~0.1 para asegurar que la señal de test (que es pequeña) lo cruce.
		// 0.1 * 1048576 (2^20) = 104857
		uint32_t umbral_test = 1048576*2;    // 1*2
		xil_printf("[INFO] Ajustando umbrales para señal de TEST (0.1)... \r\n");
		for(int ch = 0; ch < 8; ch++){
			Xil_Out32(XPAR_FIR_FILTER_0_S_AXI_UMBRAL_BASEADDR + 0x10 + (ch * 4), umbral_test);
		}
		// =========================================================================
	...
	*/
		...
		// Ajuste inicial (pon esto antes de entrar al while(1) o cámbialo en la declaración global)
		    fin = 64; // Ahora 64 bytes son exactamente 2 paquetes (32 * 2)

		    while(1){
		        if(hasNewData){             // Si la interrupción avisa que hay datos nuevos
		            hasNewData = 0;
		            ...
		            // --- BUCLE CORREGIDO PARA 8 CANALES (Salto de 32 bytes) ---
		            for (int i = inicio; i < fin; i += 32) { // Saltamos 32 bytes: el tamaño exacto de una muestra de 8 canales

		                xil_printf("[%06x] ", OFFSET_MEM_WRITE + i); // Imprime la dirección de memoria de la muestra actual

		                // Recorremos solo los 8 Canales (Status eliminado por el FIR)
		                for (int j = 0; j < 8; j++) { // Iteramos 8 veces para cubrir los 8 canales procesados

		                    // Leemos los 4 bytes de cada canal (Cada canal ocupa 32 bits en DDR)
		                    uint8_t byte0 = Xil_In8(OFFSET_MEM_WRITE + i + (j*4) + 0); // Byte menos significativo (LSB)
		                    uint8_t byte1 = Xil_In8(OFFSET_MEM_WRITE + i + (j*4) + 1);
		                    uint8_t byte2 = Xil_In8(OFFSET_MEM_WRITE + i + (j*4) + 2);
		                    uint8_t byte3 = Xil_In8(OFFSET_MEM_WRITE + i + (j*4) + 3); // Byte más significativo (MSB)

		                    // Imprimimos en Hexadecimal para que la línea no sea infinita
		                    // Formato: Byte3-Byte2-Byte1-Byte0 (Visualizamos el dato de 32 bits reconstruido)
		                    xil_printf("%02X%02X%02X%02X ", byte3, byte2, byte1, byte0); // Imprime el valor de 32 bits del canal
		                }
		                xil_printf("\r\n"); // Salto de línea tras imprimir los 8 canales de esta muestra
		            }

		            inicio = fin; // Actualizamos el inicio para la siguiente tanda
		            fin = fin + 64; // Avanzamos 64 bytes para procesar las siguientes 2 muestras en el próximo ciclo

		            if (fin >= FRAME_COUNT_MAX) { // Control del buffer circular para no salirnos de la memoria reservada
		                inicio = 0; // Reiniciamos el puntero al principio del buffer
		                fin = 64;   // Reiniciamos el límite superior
		            }
		        }
		    }
		...
		ADS_end();              // Apaga el chip ADS y libera los cables (pines) que estábamos usando

		// Imprime un mensaje de éxito en la consola para avisarnos de que el programa terminó bien
