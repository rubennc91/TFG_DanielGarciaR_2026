...
{
    // 1. Leer el estado para ver si hay errores antes de limpiar
    uint32_t status = Xil_In32(XPAR_AXI_DMA_0_BASEADDR + OFFSET_S2MM_DMASR);

    // 2. Limpiar la interrupción (Escribir 1 al bit 12 IOC_Irq)
    Xil_Out32(XPAR_AXI_DMA_0_BASEADDR + OFFSET_S2MM_DMASR, status | 0x1000);

    // 3. Controlar el avance del puntero de escritura
    frame_count++;
    if (frame_count >= FRAME_COUNT_MAX) {
        frame_count = 0;
    }

    // 4. Reprogramar la dirección de destino para la PRÓXIMA trama
    uint32_t next_addr = OFFSET_MEM_WRITE + (32 * frame_count);
    Xil_Out32(XPAR_AXI_DMA_0_BASEADDR + OFFSET_S2MMDA, next_addr);

    // 5. IMPORTANTE: Escribir la longitud SIEMPRE inicia la transferencia
