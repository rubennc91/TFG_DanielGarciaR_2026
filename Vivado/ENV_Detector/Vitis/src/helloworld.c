...
*/
	...
	*/
    ...
    while (1) {
        // 1. Solo entramos aquí si la interrupción de la DMA nos avisa
        if (hasNewData) {
        	hasNewData = 0;
        	Xil_DCacheInvalidateRange;
        			            ...
        			            // --- BUCLE CON FILTRO DE IMPRESIÓN (1 cada 160 muestras) ---
        			            for (int i = inicio; i < fin; i += 32) {

        			                // Calculamos el índice de la muestra actual (0, 1, 2...)
        			                // Solo imprimimos si el índice es múltiplo de 160
        			                if ((i / 32) % 160 == 0) {

        			                    xil_printf("[%06x] ", OFFSET_MEM_WRITE + i);

        			                    for (int j = 0; j < 8; j++) {

        			                        int32_t q_val = (int32_t)Xil_In32(OFFSET_MEM_WRITE + i + (j * 4));

        			                        if (j == 1) { // CANAL 2: Formato Q12.20
        			                        	float valor = (float)q_val / 1048576.0f;
